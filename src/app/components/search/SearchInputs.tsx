...
export function HeroSearchBanner({ onResultsFound }: { onResultsFound?: (data: any) => void }) {
  ...
  // 1. Fetch Locations and Subjects on Mount
  React.useEffect(() => {
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      // Map "All Subjects" to empty string for API
  ...
  return (
    <section className="relative w-full overflow-hidden pb-10">
      ...
      <div className="max-w-[1100px] mx-auto -mt-12 px-6 relative z-40">
        ...
        <ul className="flex bg-[#2D3D50] w-fit rounded-t-xl overflow-hidden ml-2 max-w-full overflow-x-auto">
          <FilterTab
            label="All Subjects"
            icon={GraduationCap}
        ...
        <div className="bg-white rounded-xl rounded-tl-none shadow-xl p-1.5 flex flex-col lg:flex-row items-stretch gap-1.5 border border-white">
          ...
          <div className="flex-[1.2] px-3 py-1.5 border border-slate-100 rounded-lg group hover:border-blue-400 transition-colors">
            <label className="text-[10px] text-slate-400 font-bold block uppercase tracking-tight">Location</label>
            <select 
              value={selectedLocation}
              onChange={(e) => setSelectedLocation(e.target.value)}
              className="w-full text-[14px] font-bold text-[#0F294D] outline-none bg-transparent cursor-pointer"
            >
          ...
          {/* Static UI Placeholders (Check-in / Trainees) */}
          ...
          <div className="flex-1 px-3 py-1.5 border border-slate-100 rounded-lg hover:bg-slate-50 cursor-pointer">
            ...
            <div className="flex items-center justify-between">
              <span className="text-[14px] font-bold text-[#0F294D]">1 Adult</span>
          ...
          <button 
            ...
            disabled={loading}
            className="bg-[#0066FF] hover:bg-[#0052CC] text-white flex items-center justify-center gap-2 px-8 rounded-lg font-bold transition-all active:scale-95 disabled:opacity-70 min-h-[50px]"
          >
            {loading ? (
            ...
            <span className="text-[15px]">{loading ? "Loading..." : "Search"}</span>
          </button>
        </div>
      </div>
    </section>
