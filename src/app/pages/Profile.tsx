...
/* ─────────────────────────────────────────────────────────────
   CLASS CARD — mobile-first, clean, full-width
───────────────────────────────────────────────────────────── */
function ClassCard({
  cls,
  t,
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md hover:border-[#1877f2]/20 active:scale-[0.98] transition-all duration-200"
    >
      ...
      <div className="relative h-[140px] sm:h-[160px] w-full overflow-hidden bg-gradient-to-br from-[#1877f2] to-[#42a5f5]">
        ...
        {/* Price badge */}
        <span className="absolute top-3 right-3 text-[13px] font-black text-white bg-[#1877f2]/90 backdrop-blur-sm px-3 py-1 rounded-full">
          ${cls.basePrice}
        ...
        {cls.subjects?.length > 0 && (
          <div className="absolute bottom-0 left-0 right-0 px-3 py-2 bg-gradient-to-t from-black/60 to-transparent flex gap-1.5 flex-wrap">
            {cls.subjects.slice(0, 3).map((sub) => (
              <span
                key={sub}
                className="text-[10px] font-semibold text-white/90 bg-white/20 backdrop-blur-sm px-2 py-0.5 rounded-full"
              >
                {sub}
              </span>
            ))}
      ...
      {/* Body */}
      <div className="p-4">
        <h3 className="text-[15px] font-bold text-gray-900 leading-snug line-clamp-1 mb-1">
          {cls.title}
        </h3>
        <p className="text-[12px] text-gray-500 line-clamp-2 leading-relaxed mb-3">
          {cls.description || t("គ្មានការពិពណ៌នា", "No description")}
        </p>

        {/* Students progress */}
        <div className="flex items-center justify-between text-[11px] text-gray-500 mb-1.5">
          <span className="flex items-center gap-1 font-medium">
            <Users size={11} className="text-[#1877f2]" />
            {cls.currentStudents}/{cls.maxStudents} {t("សិស្ស", "students")}
          </span>
          <span className="font-bold text-[#1877f2]">{fillPct}%</span>
        </div>
        <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-[#1877f2] to-[#42a5f5] rounded-full transition-all"
            style={{ width: `${fillPct}%` }}
          />
        </div>
...
function CoverPickerModal({
  storageKey, current, onSelect, onClose,
}: {
  storageKey: string; current: CoverPresetId;
  onSelect: (id: CoverPresetId) => void; onClose: () => void;
}) {
  return (
...
/* ─────────────────────────────────────────────────────────────
   WELCOME POPUP
───────────────────────────────────────────────────────────── */
function WelcomePopup({ role, name, onClose, t }: {
  role: "tutor" | "student"; name: string;
  onClose: () => void; t: (kh: string, en: string) => string;
}) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" style={{ animation: "popIn 0.35s cubic-bezier(0.34,1.56,0.64,1) both" }}>
        ...
        <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-500 transition-colors">
          <X size={16} />
        </button>
        <div className="p-6 pt-5 text-center">
          ...
          <h2 className="text-[22px] font-bold text-gray-900 mb-1">
            {t(`សូមស្វាគមន៍មកវិញ, ${name.split(" ")[0]}! 👋`, `Welcome back, ${name.split(" ")[0]}! 👋`)}
          </h2>
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-6">
            {pills.map((pill, i) => (
              <span key={i} className="flex items-center gap-1 text-[12px] font-semibold px-3 py-1.5 rounded-full bg-[#e7f3ff] text-[#1877f2]">
...
/* ─────────────────────────────────────────────────────────────
   EDIT PROFILE MODAL
───────────────────────────────────────────────────────────── */
function EditProfileModal({ tutor, token, onClose, onSaved }: {
  tutor: any; token: string | null; onClose: () => void; onSaved: () => void;
}) {
  const { t } = useLanguage();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(t("ធ្វើបច្ចុប្បន្នភាពដោយជោគជ័យ!", "Profile updated!"), { icon: <CheckCircle2 size={16} className="text-green-500" /> });
        onSaved(); onClose();
      } else { toast.error(t("បរាជ័យ", "Failed to update")); }
    } catch { toast.error(t("កំហុស", "Something went wrong")); }
    finally { setSaving(false); }
  };

  type FieldDef = { key: keyof EditProfileForm; label: string; icon: React.ReactNode; placeholder: string; multiline?: boolean; };
  const fields: FieldDef[] = [
    { key: "bio", label: t("ជីវប្រវត្តិ", "Bio"), icon: <FileText size={14} />, multiline: true, placeholder: t("ប្រាប់សិស្សអំពីខ្លួនអ្នក...", "Tell students about yourself...") },
    { key: "location", label: t("ទីតាំង", "Location"), icon: <MapPin size={14} />, placeholder: t("ក្រុង, ប្រទេស", "City, Country") },
    { key: "subjects", label: t("មុខវិជ្ជា (ក្បៀសបំបែក)", "Subjects (comma-sep)"), icon: <BookOpen size={14} />, placeholder: "Math, Physics" },
    { key: "experience", label: t("បទពិសោធន៍", "Experience"), icon: <Clock size={14} />, placeholder: "5 years" },
    { key: "hourlyRate", label: t("អត្រាម៉ោង ($)", "Hourly Rate ($)"), icon: <DollarSign size={14} />, placeholder: "30" },
    { key: "education", label: t("ការអប់រំ", "Education"), icon: <Award size={14} />, placeholder: "BSc Mathematics" },
    { key: "languages", label: t("ភាសា (ក្បៀសបំបែក)", "Languages (comma-sep)"), icon: <Globe size={14} />, placeholder: "English, Khmer" },
    { key: "availability", label: t("ពេលទំនេរ", "Availability"), icon: <Clock size={14} />, placeholder: "Weekdays 9am–5pm" },
  ];
  const inputCls = "w-full text-[14px] text-gray-800 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1877f2]/30 focus:border-[#1877f2] transition-all";

  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" style={{ animation: "popIn 0.3s cubic-bezier(0.34,1.56,0.64,1) both" }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-[#e7f3ff] rounded-lg flex items-center justify-center"><Edit2 size={14} className="text-[#1877f2]" /></div>
            <h2 className="text-[16px] font-bold text-gray-900">{t("កែប្រវត្តិរូប", "Edit Profile")}</h2>
          </div>
          <button onClick={onClose} className="w-8 h-8 bg-gray-100 hover:bg-gray-200 rounded-full flex items-center justify-center text-gray-500 transition-colors"><X size={15} /></button>
        </div>
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          {fields.map((field) => (
            <div key={field.key}>
              <label className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-gray-400 mb-1.5">{field.icon} {field.label}</label>
              {field.multiline ? (
                <textarea rows={3} value={form[field.key]} onChange={(e) => setForm((p) => ({ ...p, [field.key]: e.target.value }))} placeholder={field.placeholder} className={inputCls + " resize-none"} />
              ) : (
                <input type="text" value={form[field.key]} onChange={(e) => setForm((p) => ({ ...p, [field.key]: e.target.value }))} placeholder={field.placeholder} className={inputCls} />
              )}
            </div>
          ))}
        </div>
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50">
          <button onClick={onClose} className="px-4 py-2 text-[14px] font-semibold text-gray-600 hover:bg-gray-200 rounded-lg transition-colors">{t("បោះបង់", "Cancel")}</button>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-[#1877f2] hover:bg-[#166fe5] disabled:opacity-60 text-white text-[14px] font-semibold px-5 py-2 rounded-lg transition-colors">
  ...
  return (
    <>
      <div className="relative h-[200px] md:h-[300px] rounded-b-xl overflow-hidden" style={{ background: preset.style }}>
        ...
        {canEdit && (
          <button onClick={() => setShowPicker(true)} className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-white/90 hover:bg-white text-gray-800 text-[13px] font-semibold px-3 py-1.5 rounded-lg transition-all shadow-sm">
            <Camera size={13} /> Edit Cover
          </button>
        )}
      </div>
      {showPicker && <CoverPickerModal storageKey={storageKey} current={coverId} onSelect={handleSelect} onClose={() => setShowPicker(false)} />}
    </>
  );
}
...
export default function TutorProfilePage() {
  ...
  const handleRequestTutor = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        toast.success(t("ដាក់ពាក្យដោយជោគជ័យ!", "Tutor request submitted!"), { id: toastId, icon: <GraduationCap size={16} className="text-green-500" /> });
        if (data.token) { document.cookie = `token=${data.token}; path=/; max-age=86400`; setTimeout(() => window.location.reload(), 1200); }
      } else { toast.error(t("បរាជ័យ", "Request failed"), { id: toastId }); }
    } catch { toast.error(t("កំហុសបណ្តាញ", "Network error"), { id: toastId }); }
  ...
  const handlePublishChange = (published: boolean) => {
    ...
    toast.success(
      published ? t("ប្រវត្តិរូបជាសាធារណៈ!", "Profile is now Public!") : t("ប្រវត្តិរូបជាឯកជន", "Profile set to Private"),
      { icon: published ? <Globe size={16} className="text-green-500" /> : <EyeOff size={16} className="text-gray-400" /> }
    );
    fetchData();
  };
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5]" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      {showWelcome && (
        <WelcomePopup role={isTutor ? "tutor" : "student"} name={displayName} t={t}
          onClose={() => {
            setShowWelcome(false);
            toast.success(t(`សូមស្វាគមន៍, ${displayName.split(" ")[0]}!`, `Welcome, ${displayName.split(" ")[0]}!`), {
              description: isTutor ? t("ផ្ទាំងគ្រប់គ្រងរបស់អ្នករួចរាល់។", "Your dashboard is ready.") : t("រកគ្រូល្អបំផុតថ្ងៃនេះ។", "Find your perfect tutor today."),
            });
          }}
        />
      )}
      ...
      {showEditProfile && isTutor && (
        <EditProfileModal tutor={tutor} token={token} onClose={() => setShowEditProfile(false)} onSaved={fetchData} />
      )}
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 -mt-10 md:-mt-14 pb-4 border-b border-gray-200">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-3 md:gap-4">
                <div className="relative self-start">
                  <img src={avatar} alt={displayName} className="w-[120px] h-[120px] md:w-[148px] md:h-[148px] rounded-full border-4 border-white object-cover shadow-md bg-white" />
                  <button onClick={() => toast.info(t("មុខងារនឹងមកដល់ឆាប់ៗ!", "Avatar upload coming soon!"))} className="absolute bottom-1.5 right-1.5 w-8 h-8 bg-[#e4e6ea] hover:bg-[#d8dadf] rounded-full flex items-center justify-center shadow transition-colors">
                ...
                <div className="pb-1 md:pb-3">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {isTutor && tutor?.public && (
                      <span className="flex items-center gap-1 text-[12px] text-green-600 font-medium"><Globe size={11} /> {t("សាធារណៈ", "Public")}</span>
                    )}
                    {isTutor && !tutor?.public && (
                      <span className="flex items-center gap-1 text-[12px] text-gray-400"><EyeOff size={11} /> {t("ឯកជន", "Private")}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 pb-1 md:pb-3">
                {isTutor ? (
                  <div className="flex items-center gap-2">
                    ...
                    <ProfilePublish token={token} initialPublished={tutor?.public || false} onRefresh={() => handlePublishChange(!tutor?.public)} />
                  </div>
                ) : (
                  <>
                    <button onClick={handleFindTutor} className="flex items-center gap-2 bg-[#1877f2] hover:bg-[#166fe5] text-white text-[14px] font-semibold px-5 py-2 rounded-lg transition-colors">
                      <Search size={15} /> {t("រកគ្រូ", "Find a Tutor")}
                    </button>
                    <button onClick={handleRequestTutor} disabled={requestLoading} className="flex items-center gap-2 bg-white hover:bg-[#f0f2f5] disabled:opacity-60 text-[#1877f2] border border-[#1877f2] text-[14px] font-semibold px-5 py-2 rounded-lg transition-colors">
                      {requestLoading ? <Loader2 className="animate-spin" size={15} /> : <GraduationCap size={15} />}
                ...
                {[
                  ...
                  <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
                    className={`px-4 py-3 text-[15px] font-semibold border-b-[3px] transition-colors ${
                      activeTab === tab.key ? "border-[#1877f2] text-[#1877f2]" : "border-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-900 rounded-t-lg"
                    }`}>
                    {tab.label}
                  </button>
                ))}
        ...
        {isTutor && tutor && (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-2 space-y-3 px-4 sm:px-0">
              <div className="bg-white rounded-xl shadow-sm p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setShowEditProfile(true)} className="flex items-center gap-1 text-[12px] text-[#1877f2] hover:bg-[#e7f3ff] px-2 py-1 rounded-lg transition-colors font-semibold">
                ...
                {tutor?.bio ? (
                  ...
                  <button onClick={() => setShowEditProfile(true)} className="w-full text-[13px] text-[#1877f2] hover:bg-[#e7f3ff] py-2 rounded-lg transition-colors font-semibold mb-2">
            ...
            <div className="lg:col-span-3 space-y-3">
              ...
              {/* ── CLASSES SECTION — full-width on mobile ── */}
            ...
            <div className="lg:col-span-3 space-y-3">
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-6 text-center">
                  <div className="w-16 h-16 bg-[#e7f3ff] rounded-full flex items-center justify-center mx-auto mb-4"><Search size={28} className="text-[#1877f2]" /></div>
                  ...
                  <button onClick={handleFindTutor} className="inline-flex items-center gap-2 bg-[#1877f2] hover:bg-[#166fe5] text-white text-[15px] font-semibold px-7 py-2.5 rounded-lg transition-colors">
              ...
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-6 text-center">
                  <div className="w-16 h-16 bg-[#e7f3ff] rounded-full flex items-center justify-center mx-auto mb-4"><GraduationCap size={28} className="text-[#1877f2]" /></div>
                  ...
                  <button onClick={handleRequestTutor} disabled={requestLoading} className="inline-flex items-center gap-2 bg-white hover:bg-[#f0f6ff] disabled:opacity-60 text-[#1877f2] border-2 border-[#1877f2] text-[15px] font-semibold px-7 py-2.5 rounded-lg transition-colors">
                    {requestLoading ? <Loader2 className="animate-spin" size={16} /> : <GraduationCap size={16} />}
              ...
              <div className="bg-white rounded-xl shadow-sm p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <a key={i} href={link.href} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-[#f0f2f5] hover:bg-[#e7f3ff] text-gray-700 hover:text-[#1877f2] text-[13px] font-semibold transition-colors">
